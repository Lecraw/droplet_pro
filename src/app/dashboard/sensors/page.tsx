  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <p className="font-[family-name:var(--font-jetbrains-mono)] text-[9px] uppercase tracking-[0.25em] text-[#00a6e0] mb-1">Hardware</p>
          <h1 className="text-3xl font-bold text-[#0a1628] font-[family-name:var(--font-inter-tight)]">Sensor Nodes</h1>
          <p className="text-[#64748b]">Manage physical telemetry hardware deployments.</p>
        </div>
        <div className="flex items-center gap-3">
          <span className="font-[family-name:var(--font-jetbrains-mono)] text-xs text-[#64748b]">{sensors.length} nodes deployed</span>
          <button className="flex items-center gap-2 bg-[#00a6e0] text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-[#0088b8] transition-colors shadow-sm">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white/75 backdrop-blur-sm p-4 rounded-xl border border-[#e2e8f0]">
          <p className="font-[family-name:var(--font-jetbrains-mono)] text-[9px] uppercase tracking-widest text-[#94a3b8] mb-1">Total Nodes</p>
          <p className="text-2xl font-bold text-[#0a1628]">{sensors.length}</p>
        </div>
        <div className="bg-white/75 backdrop-blur-sm p-4 rounded-xl border border-[#e2e8f0]">
          <p className="font-[family-name:var(--font-jetbrains-mono)] text-[9px] uppercase tracking-widest text-[#94a3b8] mb-1">Online</p>
          <p className="text-2xl font-bold text-emerald-600">{sensors.filter(s => s.status === "Online").length}</p>
        </div>
        <div className="bg-white/75 backdrop-blur-sm p-4 rounded-xl border border-[#e2e8f0]">
          <p className="font-[family-name:var(--font-jetbrains-mono)] text-[9px] uppercase tracking-widest text-[#94a3b8] mb-1">Warnings</p>
          <p className="text-2xl font-bold text-amber-500">{sensors.filter(s => s.status === "Warning").length}</p>
        </div>
        <div className="bg-white/75 backdrop-blur-sm p-4 rounded-xl border border-[#e2e8f0]">
          <p className="font-[family-name:var(--font-jetbrains-mono)] text-[9px] uppercase tracking-widest text-[#94a3b8] mb-1">Offline</p>
          <p className="text-2xl font-bold text-red-500">{sensors.filter(s => s.status === "Offline").length}</p>
        </div>
      </div>

      <div className="bg-white/75 backdrop-blur-sm rounded-2xl border border-[#e2e8f0] shadow-sm overflow-hidden">
        <div className="p-4 border-b border-[#e2e8f0] flex justify-between items-center bg-slate-50">
          <div className="relative w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-md border border-[#e2e8f0] text-sm focus:outline-none focus:ring-1 focus:ring-[#00a6e0]"
            />
          </div>
          <span className="font-[family-name:var(--font-jetbrains-mono)] text-[9px] uppercase tracking-widest text-[#94a3b8]">
            ...
            <tbody>
              {filtered.map((sensor) => (
                <tr key={sensor.id} className="border-b border-[#e2e8f0] last:border-0 hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 font-[family-name:var(--font-jetbrains-mono)] text-[#0a1628]">{sensor.id}</td>
                  <td className="px-6 py-4 text-[#0a1628]">{sensor.location}</td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <button className="text-[#94a3b8] hover:text-[#0a1628]">
