...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <p className="font-[family-name:var(--font-jetbrains-mono)] text-[9px] uppercase tracking-[0.25em] text-[#00a6e0] mb-1">Intelligence</p>
        <h1 className="text-3xl font-bold text-[#0a1628] font-[family-name:var(--font-inter-tight)]">Analytics</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Avg PUE", value: "1.12", sub: "Industry avg: 1.58", color: "text-[#00a6e0]" },
          { label: "Water Saved YTD", value: `${(totalSaved / 1000).toFixed(0)}K gal`, sub: "+42% vs prior year", color: "text-emerald-500" },
          { label: "Cost Reduction", value: "$74K", sub: "Operating cost delta", color: "text-[#00a6e0]" },
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
          ...
          <p className="font-[family-name:var(--font-jetbrains-mono)] text-[9px] uppercase tracking-widest text-[#94a3b8] mb-1">12-Month Trend</p>
          <p className="font-bold text-[#0a1628] mb-5">Water Consumption (gal)</p>
          <div className="h-52">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={monthlyData} margin={{ left: -15 }}>
                <defs>
                  <linearGradient id="waterGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#00a6e0" stopOpacity={0.15} />
                ...
                <Area type="monotone" dataKey="water" stroke="#00a6e0" strokeWidth={2} fill="url(#waterGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="bg-white/75 backdrop-blur-sm rounded-2xl border border-[#e2e8f0] p-6 shadow-sm">
          <p className="font-[family-name:var(--font-jetbrains-mono)] text-[9px] uppercase tracking-widest text-[#94a3b8] mb-1">Efficiency Metric</p>
          <p className="font-bold text-[#0a1628] mb-5">PUE Over Time</p>
          <div className="h-52">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
        className="bg-white/75 backdrop-blur-sm rounded-2xl border border-[#e2e8f0] p-6 shadow-sm">
        <p className="font-[family-name:var(--font-jetbrains-mono)] text-[9px] uppercase tracking-widest text-[#94a3b8] mb-1">Zone Breakdown</p>
        <p className="font-bold text-[#0a1628] mb-5">Per-Zone Efficiency (%)</p>
        <div className="h-52">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={zoneData} margin={{ left: -15 }}>
              ...
              <Tooltip contentStyle={{ borderRadius: "8px", border: "1px solid #e2e8f0" }} />
              <Bar dataKey="efficiency" name="Efficiency (%)" fill="#00a6e0" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        <div className="px-6 py-4 border-b border-[#e2e8f0]">
          <p className="font-bold text-[#0a1628] text-sm">Zone Detail Report</p>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#e2e8f0]">
              {["Zone", "Flow Rate", "Avg Temp", "Efficiency", "Status"].map(h => (
                <th key={h} className="px-6 py-3 text-left font-[family-name:var(--font-jetbrains-mono)] text-[9px] uppercase tracking-widest text-[#94a3b8]">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {zoneData.map((z) => (
              <tr key={z.zone} className="border-b border-[#e2e8f0] last:border-0 hover:bg-[rgba(250,252,255,0.7)] transition-colors">
                <td className="px-6 py-3.5 font-medium text-[#0a1628]">{z.zone}</td>
                <td className="px-6 py-3.5 font-[family-name:var(--font-jetbrains-mono)] text-[#64748b]">{z.flow} L/s</td>
                <td className="px-6 py-3.5 font-[family-name:var(--font-jetbrains-mono)] text-[#64748b]">{z.temp}°C</td>
                <td className="px-6 py-3.5">
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-1.5 bg-[#e2e8f0] rounded-full overflow-hidden max-w-24">
                      <div className="h-full bg-[#00a6e0] rounded-full" style={{ width: `${z.efficiency}%` }} />
                    </div>
                    <span className="font-[family-name:var(--font-jetbrains-mono)] text-xs text-[#00a6e0]">{z.efficiency}%</span>
                  </div>
                </td>
                <td className="px-6 py-3.5">
